# Agile Artifacts Index

## Guides
- `agentic-sprint-workflow.md`
- `ai-customization-guide.md`
- `copilot-customization-guide.md`

## 00-governance
- `naming-conventions.md`
- `artifact-lifecycle.md`
- `roles-and-raci.md`

## 01-templates
### concept
- `concept-artifacts.template.md`

### inception
- `inception-artifacts.template.md`

### iteration
- `iteration-artifacts.template.md`
- `sprint-pack.template.md`

### release
- `release-artifacts.template.md`
- `release-pack.template.md`

### maintenance
- `maintenance-artifacts.template.md`

### retirement
- `retirement-artifacts.template.md`

## 02-phase-artifacts
### concept
- `product-vision-board.md`
- `initial-product-backlog-epics.md`
- `feasibility-risk-snapshot.md`

### inception
- `architecture-decision-record-log.md`
- `user-stories-and-acceptance-criteria.md`
- `technical-blueprint.md`

### iteration
- `sprint-goal.md`
- `sprint-backlog.md`
- `definition-of-done.md`
- `sprint-ceremonies-tracker.md`

### release
- `qa-strategy-and-test-plan.md`
- `test-cases.md`
- `release-notes-template.md`
- `go-no-go-checklist.md`

### maintenance
- `maintenance-backlog.md`
- `monitoring-and-incident-response.md`
- `continuous-improvement-plan.md`

### retirement
- `retirement-strategy.md`
- `migration-runbook.md`
- `decommission-checklist-and-communications.md`

## 03-delivery-records
### release-01
- `release-pack.md`
- `sprint-01.md`
- `sprint-02.md`

### release-02
- `release-pack.md`
- `sprint-03.md`
- `sprint-04.md`

### release-03
- `release-pack.md`
- `sprint-05.md`
- `sprint-06.md`

### archive
- `archive.md`

## Tracking
- `artifact-registry.md`
