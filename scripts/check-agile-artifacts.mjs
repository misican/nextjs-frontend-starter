...
if (!existsSync(archivePath)) {
	errors.push('Missing docs/specs/agile-artifacts/03-delivery-records/archive.md');
}
...
const registryPaths = Array.from(registryContent.matchAll(/\|\s(docs\/specs\/agile-artifacts\/03-delivery-records\/[^|]+)\s\|/g)).map((match) => match[1]);
for (const registryPathValue of registryPaths) {
	const absolute = join(rootDir, registryPathValue);
	if (!existsSync(absolute)) {
		errors.push(`artifact-registry.md references missing path ${registryPathValue}`);
	}
}
